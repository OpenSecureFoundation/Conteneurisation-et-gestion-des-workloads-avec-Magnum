from services.openstack_client import get_connection

HYBRID_TEMPLATE = """
heat_template_version: 2021-04-16
description: Stack hybride VM + Cluster Magnum

parameters:
  cluster_template_id:
    type: string
  cluster_name:
    type: string
    default: hybrid-cluster
  master_count:
    type: number
    default: 1
  node_count:
    type: number
    default: 2
  vm_flavor:
    type: string
    default: m1.small
  vm_image:
    type: string
  keypair:
    type: string

resources:
  app_network:
    type: OS::Neutron::Net
    properties:
      name: hybrid-net
  app_subnet:
    type: OS::Neutron::Subnet
    properties:
      network: { get_resource: app_network }
      cidr: 192.168.100.0/24
      dns_nameservers: [8.8.8.8]
  classic_vm:
    type: OS::Nova::Server
    properties:
      name: hybrid-vm
      flavor: { get_param: vm_flavor }
      image: { get_param: vm_image }
      key_name: { get_param: keypair }
      networks:
        - network: { get_resource: app_network }
  container_cluster:
    type: OS::Magnum::Cluster
    properties:
      name: { get_param: cluster_name }
      cluster_template: { get_param: cluster_template_id }
      master_count: { get_param: master_count }
      node_count: { get_param: node_count }
      keypair: { get_param: keypair }

outputs:
  vm_ip:
    value: { get_attr: [classic_vm, first_address] }
  cluster_id:
    value: { get_resource: container_cluster }
"""

def list_stacks():
    conn = get_connection()
    return [_fmt(s) for s in conn.orchestration.stacks()]

def get_stack(sid):
    conn = get_connection()
    s = conn.orchestration.find_stack(sid)
    return _fmt(s) if s else None

def create_hybrid_stack(data):
    conn = get_connection()
    s = conn.orchestration.create_stack(
        name=data["stack_name"], template=HYBRID_TEMPLATE,
        parameters={
            "cluster_template_id": data["cluster_template_id"],
            "cluster_name": data.get("cluster_name", "hybrid-cluster"),
            "master_count": int(data.get("master_count", 1)),
            "node_count": int(data.get("node_count", 2)),
            "vm_flavor": data.get("vm_flavor", "m1.small"),
            "vm_image": data.get("vm_image", ""),
            "keypair": data.get("keypair", ""),
        },
        timeout_mins=int(data.get("timeout_mins", 60)),
        disable_rollback=False,
    )
    return _fmt(s)

def create_custom_stack(data):
    conn = get_connection()
    s = conn.orchestration.create_stack(
        name=data["stack_name"], template=data["template"],
        parameters=data.get("parameters", {}),
        timeout_mins=int(data.get("timeout_mins", 60)),
    )
    return _fmt(s)

def delete_stack(sid):
    conn = get_connection()
    conn.orchestration.delete_stack(sid)
    return {"status": "deleted", "id": sid}

def get_stack_resources(sid):
    conn = get_connection()
    return [{"name": r.name, "type": r.resource_type, "status": r.resource_status,
             "physical_id": r.physical_resource_id,
             "updated_time": str(r.updated_time) if r.updated_time else None}
            for r in conn.orchestration.resources(sid)]

def get_stack_events(sid):
    conn = get_connection()
    evts = list(conn.orchestration.events(sid))
    return [{"resource_name": e.resource_name, "status": e.resource_status,
             "reason": e.resource_status_reason,
             "time": str(e.event_time) if e.event_time else None}
            for e in evts[-20:]]

def get_hybrid_template():
    return HYBRID_TEMPLATE

def _fmt(s):
    return {"id": s.id, "name": s.name, "status": s.status,
            "description": getattr(s, "description", None),
            "created_at": str(s.created_at) if getattr(s, "created_at", None) else None,
            "updated_at": str(s.updated_at) if getattr(s, "updated_at", None) else None,
            "outputs": getattr(s, "outputs", []),
            "parameters": getattr(s, "parameters", {})}
